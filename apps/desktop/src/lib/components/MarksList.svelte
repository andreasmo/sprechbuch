<script lang="ts">
  ...
  const ICON: Record<MarkKind, string> = { retake: "⟲", bookmark: "★", note: "✎" };
  const LABEL: Record<MarkKind, string> = { retake: "Retakes", bookmark: "Lesezeichen", note: "Notizen" };
...
<section class="panel card">
  <div class="card-head">
    <h2>Retakes, Lesezeichen und Notizen <span class="muted count">{fmt(rows.length)}</span></h2>
    {#if rows.length}
      <div class="actions">
  ...
  {#if !rows.length}
    <p class="muted small empty">
      ...
      am aktuellen Satz – hier sammelt sich dann die Liste für die Nachbearbeitung.
    </p>
  {:else}
    <div class="filters" role="group" aria-label="Filter">
      <button class="ghost" class:active={filter === "all"} onclick={() => (filter = "all")}>Alle</button>
      {#each ["retake", "bookmark", "note"] as const as k (k)}
    ...
    <div class="table-wrap">
      <table>
        <thead><tr><th></th><th>Stelle</th><th>Text</th><th>Notiz</th><th></th></tr></thead>
        <tbody>
          {#each shown as r (r.id)}
            <tr>
              <td class="icon {r.type}" title={LABEL[r.type]}>{ICON[r.type]}</td>
              ...
              <td class="note">
                ...
                {:else}
                  <input value={r.note} placeholder={r.type === "retake" ? "Grund, z. B. versprochen" : ""} aria-label="Notiz"
