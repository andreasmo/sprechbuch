...
import {
  ...
  setProgress, sha256Hex, type Book, type Edit, type JournalEntry, type Patch,
} from "@sprechbuch/core";
import { FileConflictError, FileMissingError, siblingPath, type FileStamp, type Platform } from "../platform";
import { openHbook, packHbook } from "../worker/protocol";
import { saveSnapshot, saveSource } from "./persist";
import { SearchState } from "./search.svelte";
import { settings } from "./settings.svelte";
import { mayHaveChanged } from "./sync";
...
export class BookSession {
  ...
  async save(saveAs = false, opts: { quiet?: boolean; force?: boolean } = {}): Promise<boolean> {
    ...
    try {
      ...
      if (!files) {
        // Web: herunterladen; der Hash erkennt die Datei beim erneuten Öffnen wieder
        const name = await this.platform.saveFile(bytes, `${this.fileName}.hbook`, "hbook");
        if (!name) return false;
        this.#saved(rev, posKey, name, { size: bytes.length, modifiedMs: Date.now(), sha256: await sha256Hex(bytes) });
        this.notify(`Heruntergeladen: ${name}`);
        return true;
      }
